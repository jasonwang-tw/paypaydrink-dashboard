<template>
  <div id="order" class="text-main-500">
        ...
        <div v-if="orderItem.number == detailCurrent">
          ...
          >
            ...
            >
              <div class="w-full mb-5 md:mb-0 md:flex">
                <div
                  class="flex-shrink-0 hidden mr-3 overflow-hidden drinkImg rounded-3xl md:block"
                >
                ...
                <div class="w-full">
                  ...
                  <div class="md:flex">
                    <div class="w-full text-sm">
                      <div class="flex">
                        <span class="flex-shrink-0 drinkBefore text-lightblue-500">內含配料</span>
                        <div>
                    ...
                    <div
                      class="flex items-center justify-between flex-shrink-0 mt-1.5 text-sm md:mt-0"
                    >
                      <div class="flex font-normal md:mx-10 qty">
                        <span class="block text-lightblue-500 drinkBefore md:hidden">數量</span>x
                        {{ i.qty }}
                      </div>
                      <div class="text-xl font-normal md:mr-5 price text-subyellow-500">
                        ${{ i.price }}
                      </div>
                    </div>
                  </div>
          ...
          <div class="text-right">
            ...
            <p class="flex items-center justify-end text-sm">
              <span class="mr-5 text-lightblue-500">折扣碼優惠 9折</span>$330
            </p>
            <big class="flex items-center justify-end font-medium text-subyellow-500"
              ><span class="mr-5 text-base">總計</span>$ 5787878</big
            >
          </div>
        </div>
      </div>
    ...
    <popup :class="{ popupHidden: QRcodeShow }">
      ...
      <template v-slot:content>
        ...
        <div class="flex items-center justify-center">
          <h5>
            <i class="mr-2 text-xl pay-list text-lightblue-500"></i><strong>訂單</strong>
            <span class="ml-2 text-blue-900">001</span>
          </h5>
        </div>
    ...
    <popup :class="{ popupHidden: shareLink }">
      ...
      <template v-slot:content>
        ...
        <div class="flex mb-10">
          ...
          <div
            @click="copyUrl"
            class="flex-shrink-0 text-blue-100 cursor-pointer btn hover:text-main-500"
          >
            複製連結
          </div>
        </div>
    ...
    <!-- 給評 -->
    <popup :class="{ popupHidden: ratingView }">
      <template v-slot:title>
        <h4>評價此飲品</h4>
      </template>
      <template v-slot:content>
        <p>請留下至少 30 個字的評價，以獲得 <span class="text-subyellow-100">1 PayPoint</span></p>
        <div class="flex items-center justify-center my-5 text-lightblue-high">
          <div v-for="(i, index) in comment" :key="index" class="mx-2">
            <input
              type="radio"
              name="rating"
            ...
            /><label :for="i.value" class="border border-lightblue-high btn-sm"
              ><i class="mr-2 pay-start"></i>{{ i.text }}</label
            >
          </div>
        </div>
      ...
      <template v-slot:btn>
        <div class="flex justify-center mt-5 functionBtn">
          <div class="btn btn-dark-blue" @click=";(ratingView = true), (complate_rating = false)">
            送出
          </div>
        </div>
      </template>
    ...
    <!-- 完成-->
    <popup :class="{ popupHidden: complate_rating }">
      <template v-slot:title>
        <i class="block mb-5 text-5xl pay-good"></i>
        <h4>完成評價</h4>
      </template>
      <template v-slot:content>
        <p>恭喜完成評價，您將獲得 <span class="text-subyellow-100">1 PayPoint</span>作為獎勵</p>
      </template>
      <template v-slot:btn>
        <div class="flex justify-center mt-5 functionBtn">
          <div class="btn btn-dark-blue" @click="complate_rating = true">確認</div>
        </div>
      </template>
    </popup>
  </div>
